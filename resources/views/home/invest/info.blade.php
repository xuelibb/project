...
<style>
	*{
	...
	.conten .box .pawDiv:nth-child(6) input.paw{
		border: none;
	}
	.conten .box input.paw:focus{outline:0;}
	.conten .box .pawDiv{
		display: inline-block;
		line-height: 30px;
		width: 31px;
...
<script  src="Public/web/css/layer.css"></script>
<script>
</script>
<script>
    	...
    	<div class="ofh Invest-detail-box">
                            ...
                            <div class="fl dib">
                            	<span>出借进度：</span>
                           		<div class="fl dib trade-time-box">
                                    ...
                                    <div class="fl dib trade-time-percent"  id='wz_jindu' ><?php echo    sprintf("%.2f",($info['lend_repay_money']/$info['lend_money']*100)); ?>%</div>
                                </div>
                            </div>
                        </div>
                    </dd>
                    ...
                    <dd>
                    	<div class="mt10 mb10">
                            <label class="check-box fstyle1 fc3"><input class="checkbox" checked="checked" id="aggrement" type="checkbox"><i class=""></i> 我已阅读并同意
                                                        	<a class="fc_blue1" href="javascript:void(0);" onclick="view_protoco();">《出借协议》</a>                            </label>
                        </div>
                        <div class="login-btn-box2">
							<a class="" href="login"><input value="立即出借" class="login_btn login_btn2" id="do_tender" type="button"></a>
                        </div>
                    </dd>
			...
			<?php	}else{      ?>
					...
					<dd class="content-right-ifo1" id='youa'  >
						...
						<form method="POST" action="/web/usertrade/investment.html" target="_blank" id="submit_form">
							<div class="Invest-wanted-price">
								<input onkeyup='this.value=this.value.replace(/\D/gi,"")'  class="fstyle1" name="order_amount" id="tender_amount" placeholder="最低100元起投" type="text">
					...
					<dd id='youb'>
						<div class="mt10 mb10">
							<label class="check-box fstyle1 fc3"><input class="checkbox aggrement" checked="checked" id="aggre" type="checkbox"><i class=""></i> 我已阅读并同意
								<a class="fc_blue1" href="javascript:void(0);" onclick="view_protocol();">《出借协议》</a>                            </label>
						</div>
						<div class="conten" style="display: none" >
							<div class="box"></div><div class="point">请输入6位数字密码</div>
							<div class="errorPoint">请输入数字！</div>

						</div>
						<div class="login-btn-box2" id="lj">
							<input value="立即出借"  class="login_btn login_btn2"  type="button">
						</div>
						<div class="login-btn-box2" id="qr" style="display: none"  >
							<input value="确认投资"  class="login_btn login_btn2 getPasswordBtn" id="chujie" type="button">
						</div>
				...
				<?php }else{  ?>
					<dd>
					<div class="tc mt38"  ><img src="Public/web/images/r3_bg.png"></div><div class="fs14 fc3 tc lh34 mb15"><p>完成时间：<?php echo date('Y-m-d H:i:s',$one['invest_time']);  ?></p>
						<p>亲,同一个标只能投一次哦~~~！</p></div>
					</dd>
					<?php } ?>
			<?php   } ?>
		  </dl>
...
function getDetail(val) {
	...
	if(url != "") {
		...
		$.ajax({
			url: url,
			type:'post',
			data:{'lend_id':lend_id,'_token':_token},
			dataType: 'html',
...
//验证投资金额
function yztzje(){
    var input=$("#tender_amount").val();
    if(input>=1){
        if(/^[1-9][0-9]*0{2}$/.test(input)==true){
           return  true;
        }else{
            layer.alert("起投金额为100,以及100的倍数哦亲~~!!", {icon:0});
            return false;
        }
    }
    else{
   ...
   if(yztzje()==true&&xy()==true){
           ...
           $.ajax({
               ...
               success: function (result) {
                   user_info( <?php echo session('user_id') ?> );
                   if(result==88){
                       layer.alert("亲~~密码错误请重新输入哦！", {icon:4});
                       return false;
				   }else if (result == 0) {
                       layer.alert("亲，不能超额投资哦!!!", {icon:6});
                       return false;
                   } else if (result == 1) {
                       $("#youa").hide();
                       $("#youb").hide();
...
//<div class="fs14 fc3 tc mt20 lh34">完成耗时:1分钟抢光</div>
                       ...
                       layer.alert("亲，投资失败亲~~!!", {icon:5});
                       return false;
                   } else if (result == 3) {
                       layer.alert("亲~余额不足，请充值哦", {icon:5});
                       return false;
                   } else if (result == 4) {
                       layer.alert("最少投资100哦,亲~~!!", {icon:0});
                       return false;
                   }
...
function inf(id){
	...
	$.ajax({
			...
			success:function(result){
				...
				var jindu=(lend_repay_money/lend_money*100).toPrecision(3);
